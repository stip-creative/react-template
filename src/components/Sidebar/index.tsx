...
import classesQuery from "../../queries/classes.graphql";
...
const Sidebar: FunctionComponent<ISelecters> = ({ classes, course_types, subjects, coursesMeta }) => {
    const [getAvailableCourses] = useLazyQuery(classesQuery);
    ...
    useEffect(() => {
        const getAvailableItems = async () => {
            ...
            const classesResult = await getAvailableCourses({ variables });
            const subjectsResult = await getAvailableCourses({
                variables: {
                    course_type: variables.course_type,
                },
            });

            const selectersForClasses: ISelecters = sidebarFiltersTransform(classesResult.data.sidebar.edges);
            const selectersForSubjects: ISelecters = sidebarFiltersTransform(subjectsResult.data.sidebar.edges);
        ...
        getAvailableItems();
    }, [getAvailableCourses, sidebarDate]);
    ...
    const onButtonClick = () => {
        const course = coursesMeta.find(
            (meta: ICourseMeta) => meta.class === sidebarDate.class.value && meta.course_type === sidebarDate.course_type.value && meta.subject === sidebarDate.subject.value
        );

        // eslint-disable-next-line no-underscore-dangle
        if (course?._meta?.uid) {
            // eslint-disable-next-line no-underscore-dangle
            navigate(`/course/${course?._meta?.uid}`);
            dispatch(updateIsOpenSidebar(!isOpenSidebar));
        }
    };
